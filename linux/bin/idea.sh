...
if [ -z "$JDK" ] && [ -s "${XDG_CONFIG_HOME:-$HOME/.config}/JetBrains/IntelliJIdea2020.1/idea.jdk" ]; then
  USER_JRE=$("$CAT" "${XDG_CONFIG_HOME:-$HOME/.config}/JetBrains/IntelliJIdea2020.1/idea.jdk")
  if [ ! -d "$USER_JRE" ]; then
...
if [ -z "$JDK" ] || [ ! -x "$JAVA_BIN" ]; then
  X86_JRE_URL="https://download.jetbrains.com/idea/jbr-for-201.7223.91-linux-x86.tar.gz"
...
elif [ -r "${XDG_CONFIG_HOME:-$HOME/.config}/JetBrains/IntelliJIdea2020.1/idea$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/JetBrains/IntelliJIdea2020.1/idea$BITS.vmoptions"
